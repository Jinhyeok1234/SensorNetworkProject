...
class WSNClient(cmd.Cmd):
    ...
    def _process_frame(self, frame):
        ...
        elif am_id == AMType.REPORT_MSG:
            if len(payload) >= 9:
                node_id, seq_num, timestamp, setting = struct.unpack(">HHIB", str(payload[:9]))
                
                active_targets = TargetCommand.get_mask_targets(setting)
                num_values = len(active_targets)
                expected_len = 9 + num_values * 2
                
                if len(payload) >= expected_len:
                    values = struct.unpack(">" + "H" * num_values, str(payload[9:expected_len])) if num_values > 0 else ()
