...
module WSNC {
  uses {
    ...
    interface AMSend as RadioCmdSend;
    interface Receive as RadioCmdReceive;

    // Radio for response packet
    interface AMSend as RadioResSend;
    interface Receive as RadioResReceive;
    
    // Radio for report packet
    interface AMSend as RadioReportSend;
    interface Receive as RadioReportReceive;
...
implementation {
  ...
  // send response of command by radio (Sensor Node -> Base Station)
  void send_response(uint8_t target, uint16_t val) {
    ...
    res->timestamp = call DataTimer.getNow();
  ...
  // send report by radio (Sensor Node -> Base Station)
  void send_report() {
    report_msg_t* rep;
    uint8_t i;
    if (radio_busy) {
    ...
    rep = (report_msg_t*)call RadioPacket.getPayload(&radio_pkt, sizeof(report_msg_t));
    if (rep == NULL) {
      is_reporting = FALSE;
      return;
    }
    ...
    rep->timestamp = call DataTimer.getNow();
    ...
    if (call RadioReportSend.send(BASE_STATION_ID, &radio_pkt, sizeof(report_msg_t)) == SUCCESS) {
      radio_busy = TRUE;
    } else {
      is_reporting = FALSE;
    }
  ...
  event message_t* SerialCmdReceive.receive(message_t* msg, void* payload, uint8_t len) {
    if (TOS_NODE_ID == BASE_STATION_ID) {
      ...
      src = (cmd_msg_t*)payload;
      dest = (cmd_msg_t*)call RadioPacket.getPayload(&radio_pkt, sizeof(cmd_msg_t));
      if (dest == NULL) return msg;

      memcpy(dest, src, sizeof(cmd_msg_t));
      
      if (!radio_busy) {
        // send to dest_node (0xFFFF : broadcast) (otherwise : unicast)
        if (call RadioCmdSend.send(src->dest_node, &radio_pkt, sizeof(cmd_msg_t)) == SUCCESS) {
          radio_busy = TRUE;
        }
      }
    }
  ...
  // receive command by radio (Base Station -> Sensor Node)
  event message_t* RadioCmdReceive.receive(message_t* msg, void* payload, uint8_t len) {
    if (TOS_NODE_ID != BASE_STATION_ID) {
      cmd_msg_t* cmd = (cmd_msg_t*)payload;
  ...
  // receive response by radio (Sensor Node -> Base Station)
  event message_t* RadioResReceive.receive(message_t* msg, void* payload, uint8_t len) {
    if (TOS_NODE_ID == BASE_STATION_ID) {
  ...
  // receive periodic report by radio (Sensor Node -> Base Station)
  event message_t* RadioReportReceive.receive(message_t* msg, void* payload, uint8_t len) {
    if (TOS_NODE_ID == BASE_STATION_ID) {
  ...
  event void RadioCmdSend.sendDone(message_t* msg, error_t err) { radio_busy = FALSE; }
  event void RadioResSend.sendDone(message_t* msg, error_t err) { radio_busy = FALSE; }
  event void RadioReportSend.sendDone(message_t* msg, error_t err) { 
    radio_busy = FALSE;
    is_reporting = FALSE; 
  }
