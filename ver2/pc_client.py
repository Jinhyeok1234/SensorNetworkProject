...
class WSNClient(cmd.Cmd):
    ...
    def _process_frame(self, frame):
        ...
        if am_id == AMType.RES_MSG or am_id == AMType.CTP_RES:
            ...
            if len(payload) >= 11:
                node_id, seq_num, timestamp, parent_id, setting = struct.unpack(">HHIHB", payload[:11])
                ...
                expected_len = 11 + num_values * 2
                
                if len(payload) >= expected_len:
                    values = struct.unpack(">" + "H" * num_values, str(payload[11:expected_len])) if num_values > 0 else ()
                    ...
                    self.logger.info("[Node %d][Seq:%d] REPORT: %s", node_id, seq_num, " | ".join(report_strs))
                else:
        ...
        for child, parent in self.topology.items():
            ...
            def print_tree(node, prefix="", is_last=True, is_root=True):
                if is_root:
                    ...
                    self.logger.info("%s%sNode %d", prefix, branch, node)
